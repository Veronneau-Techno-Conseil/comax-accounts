...
namespace CommunAxiom.Accounts.Controllers.Management
{
    ...
    public class EcosystemVersionTagConfigController : Controller
    {
        private AccountsDbContext _context;
        public EcosystemVersionTagConfigController(AccountsDbContext context)
        {
            _context = context;
        }
        ...
        [HttpGet]
        [Route("/management/ecosystemversion/{ecover}/tag/{tagid}/cfg/{id}/details")]
        public async Task<ActionResult> Details(int ecover, int tagid, int id)
        {
            var value = await _context.Set<EcosystemVersionTagConfig>()
                        .Include(x=>x.EcosystemVersionTag).ThenInclude(x => x.EcosystemVersion).ThenInclude(x => x.Ecosystem)
                        .FirstOrDefaultAsync(x => x.Id == id && x.AppVersionTagId == tagid && x.EcosystemVersionId == ecover);
            if (value == null)
                return NotFound();
            value.Localize();
